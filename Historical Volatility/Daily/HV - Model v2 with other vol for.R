...
x <- log(get(stock[a])[,4]-lag(get(stock[a])[,4],k=1)/lag(get(stock[a])[,4],k=1))
...
plotRegressionError(as.numeric(outputs[train]),fit$fitted.values, main = paste("Fitted Value vs. Actual Value"))
