...
par(mfrow=c(2,1))
xy <- get(stock)[,4]
xy1 <- length(xy)-10 #the thing i changed
xy2 <- length(xy)
xy <- xy[xy1:xy2,]
plot(xy, main = paste(stock, " - Prior to Forecast"),sub = get(stock)[length(get(stock)[,1]),4])
plot(final, type = "l", main = paste(stock, " - Forecasted 10 days - ",today()),
